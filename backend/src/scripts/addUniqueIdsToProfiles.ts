...
async function addUniqueIdsToProfiles() {
  try {
    ...
    // Find all profiles without uniqueId
    const profilesWithoutId = await PlayerProfile.find({
    ...
    for (const profile of profilesWithoutId) {
      try {
        ...
        console.log(
          `✅ Added unique ID "${uniqueId}" to profile: ${profile.inGameName}`
        );
        successCount++;
      } catch (error: any) {
        console.error(
          `❌ Error adding unique ID to profile ${profile.inGameName}:`,
          error.message
        );
        errorCount++;
      }
